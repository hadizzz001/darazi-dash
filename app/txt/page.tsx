...
const ManageCategory = () => {
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch('/api/txt', { method: 'GET' });
      if (res.ok) {
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`/api/txt?id=${encodeURIComponent(editFormData.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      console.error('Error:', error);
      setMessage('An error occurred while updating the Text.');
    }
  };
  ...
  const handleDelete = async (id) => {
    if (confirm(`Are you sure you want to delete this Text?`)) {
      try {
        const res = await fetch(`/api/txt?id=${encodeURIComponent(id)}`, {
          method: 'DELETE',
        });
        if (res.ok) {
  ...
  return (
    <div className="container mx-auto p-4 text-[13px]">
      <h1 className="text-2xl font-bold mb-4">{editMode ? 'Edit Text' : 'Add Text'}</h1>
